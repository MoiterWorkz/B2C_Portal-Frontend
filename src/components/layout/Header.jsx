...
const Header = ({ toggleSidebar }) => {
  ...
  const getUserInitials = (fullName) => {
    ...
    // More than one → take first letters of first two words
    return (parts[0][0] + parts[1][0]).toUpperCase();
  };
  const username = dashBoardData?.customerName
    ? getUserInitials(dashBoardData.customerName)
    : "";
    ...
    >
      <div className="flex items-center justify-between px-4 lg:px-6 h-16 border-bottom">
        ...
        <div className="flex items-center gap-3 relative">
          <button className="inline-flex items-center justify-center hover:text-accent-foreground rounded-md px-2 sm:px-3 md:px-4 py-1.5 sm:py-2 md:py-2.5 gap-1 sm:gap-2 transition-all duration-300 hover:scale-105 Language-button">
          ...
          >
            <span
              ...
              style={{
                backgroundColor: "var(--user-icon-background-color)",
              }}
            >
              {username}
            </span>
